...
class NewBikeDAGANDataset(DAGANDataset):
    def __init__(self, batch_size, last_training_class_index, reverse_channels, num_of_gpus, gen_batches):
        super(NewBikeDAGANDataset, self).__init__(batch_size, last_training_class_index, reverse_channels, num_of_gpus,
                                                  gen_batches)
    def load_dataset(self, gan_training_index):
        self.x = np.load("datasets/gaze.h5")
        x_train, x_test, x_val = self.x[:20], self.x[20:50], self.x[50:]
        x_train = x_train[:gan_training_index]

        return x_train, x_test, x_val


class dog(DAGANDataset):
    def __init__(self, batch_size, last_training_class_index, reverse_channels, num_of_gpus, gen_batches):
        super(dog, self).__init__(batch_size, last_training_class_index, reverse_channels, num_of_gpus,
                                                  gen_batches)

    def load_dataset(self, gan_training_index):

        self.x = np.load("datasets/train.npy")
        self.x = self.x / np.max(self.x)
        self.x = np.reshape(self.x, newshape=(2354, 100, 64, 64, 3))
        x_train, x_test, x_val = self.x[:50], self.x[51:70], self.x[71:]
        x_train = x_train[:gan_training_index]
